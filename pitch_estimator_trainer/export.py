"""Convert a pitch_estimator_trainer checkpoint into a Beatrice-compatible
pretrained file.

The training checkpoint contains:
    {"step", "pitch_estimator", "optim", "scaler", "args"}

Beatrice's loader (beatrice_trainer/__main__.py: prepare_training) expects:
    {"pitch_estimator": <state_dict>}

This script strips everything else (optimizer state, etc.) and verifies the
result loads cleanly into a fresh PitchEstimator.

Usage:
    uv run python -m pitch_estimator_trainer.export <input_ckpt> <output.pt>
...
def main() -> None:
    ...
    if "pitch_estimator" not in ckpt:
        raise SystemExit(f"input ckpt has no 'pitch_estimator' key (keys={list(ckpt)})")
    state_dict = ckpt["pitch_estimator"]
    step = ckpt.get("step", "?")
    print(f"  step: {step}, tensors: {len(state_dict)}")

    # Sanity check: must load cleanly into a fresh PitchEstimator().
    ...
    torch.save({"pitch_estimator": state_dict}, args.output)
    ...
    print(f'    "pitch_estimator_file": "{args.output.as_posix()}"')
