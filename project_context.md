...
### 2.11 — Richness root-cause = NEURAL COLLAPSE from hard CE, not architecture (2026-06-04/05)
- **The architecture is NOT the bottleneck.** Our `PhoneExtractor` is the *identical* 323-tensor network as jp_122 (loads with `missing=0 unexpected=0`), and jp_122 reaches `effective_rank ≈ 20.7` with it. The 128-dim output is not binding either (we sit at 8–14). So whatever limits richness is the **training signal**, not the model capacity. Do not waste time widening/redesigning the extractor — it can demonstrably be rich.
- **Mechanism = neural collapse.** Hard cross-entropy to K discrete units drives the terminal-phase collapse (Papyan–Han–Donoho): within-unit variance shrinks toward zero and class means align to a tight simplex. This destroys the sub-phonemic/coarticulation detail that `effective_rank` measures, and it **worsens with more steps** — exactly the observed K=500 erosion (`eff_rank` 14.25 @40k → 11.65 @153k). The early-stop monitor is only a band-aid for this.
- **Apples-to-apples richness (same 40 clips, `watch_richness.score_checkpoint`)**: `jp_122` eff_rank **20.72** / temporal_contrast 0.459 / pairwise_cos 0.072 — the richness target. `K500_bank@153k` 11.12 / 0.369 / 0.097. Use these as the reference scale.
- **jp_122 feature scale is TINY** (per-dim std median **0.027**, max 0.064, overall std 0.030). Richness lives in *how variance is distributed across dims*, NOT magnitude. ⇒ any variance-style regularizer (VICReg) must be **scale-invariant** (normalize by global std first); a raw `gamma=1` target would try to inflate features ~33× and destroy the geometry.
- **Monitoring + auto-terminate** (`phone_extractor_trainer/watch_richness.py`): scores every new numbered checkpoint's `effective_rank` over a FIXED 40-clip held-out set (seed 1234), logs `richness_monitor.csv`, and SIGTERMs training after `--patience` consecutive checkpoints **below the running max**. Numbered checkpoints save every 10k steps. Caveat: this guards against *erosion* (high→low); it will NOT fire on a run that *recovers from collapse* (low→high).
- **❌ FAILED bundled experiment (2026-06-04, `outputs/phone_extractor_en_v4_k2000_rich`)**: jp_122 warm-start + **all three richness fixes at once** (MLP head + output-noise + scale-invariant VICReg, lr 1e-4, K=2000). RESULT = **worst of all** (eff_rank 7.97, pairwise_cos **0.844** = heavily collapsed). CSV shows **total collapse early** (cos=1.0000 @10k, eff_rank 0.04) then a slow crawl back to 7.97 @200k — never recovering jp level. **Diagnosis = head-shock**: a fresh random head's CE gradients nuked the warm-started body in the first few k steps. **Methodology error**: changed 4 variables simultaneously (warm-start + MLP head + noise + VICReg) → confounded, un-attributable. **Lesson: ablate ONE variable at a time.**
- **`train_v4.py` now exposes (opt-in, defaults reproduce plain Soft-VC)**: `--head-mlp-dim` (MLP CE head; SimCLR projection-head trick), `--output-noise` (jp_122 recipe, on the CE path only), `--vicreg-var/--vicreg-cov/--vicreg-gamma` (scale-invariant anti-collapse on clean exported features), `--freeze-body-steps` + `--body-lr-scale` (frozen-body head-warmup with a 2-group optimizer: head `lr_scale=1.0`, body `lr_scale=body_lr_scale`).
- **🎯 CURRENT (2026-06-05, `outputs/phone_extractor_en_v4_k2000_jpfreeze`)**: clean isolation of the head-shock fix — warm-start jp_122, **freeze body 5000 steps** (linear head learns the English units on jp's rich features), then **unfreeze at body LR 2e-5** (`--lr 2e-4 --body-lr-scale 0.1`). NO VICReg/noise/MLP. Early signal: with the body frozen, `eff_rank` stays ~26 (richness intact) while `acc` climbs — the opposite of the bundled run's collapse. **The decisive test is whether richness HOLDS near ~20 after the body unfreezes at 5k.** If it holds while English correctness improves → success. If it erodes → re-introduce the gentle regularizers one at a time. Reminder: `effective_rank` is a proxy VICReg can game with noise dims — CE stays primary and the **blind listening test still decides promotion** (2.9).
